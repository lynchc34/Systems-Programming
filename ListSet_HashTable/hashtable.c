...
{
  struct hashtable * hashresult = malloc(sizeof(struct hashtable));
  hashresult -> size = size;
  hashresult -> table = malloc(sizeof(struct hashnode*)*size);

  for (int count =0; count < size; count++)
  {
    hashresult-> table[count] = listset_new();
  }
  return hashresult;
}
...
{
  int hashresult1 = hash_string(item, this->size);
  listset_add(this->table[hashresult1], item);
  return;
}
...
{
  int hashresult1 = hash_string(item, this->size);
  return listset_lookup(this->table[hashresult1],item);
}
...
{
  int hashresult1 = hash_string(item, this->size);
  listset_remove(this->table[hashresult1], item);
  return;
}
